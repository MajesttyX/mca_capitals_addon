...
public class OpenDeclarationOfSeparationPacket {
    ...
    public static OpenDeclarationOfSeparationPacket decode(FriendlyByteBuf buffer) {
        return new OpenDeclarationOfSeparationPacket(buffer.readUUID(), buffer.readUtf(), buffer.readUtf(), buffer.readUtf());
    }

    public static void handle(OpenDeclarationOfSeparationPacket packet) {
        DeclarationOfSeparationClient.open(packet.targetId, packet.targetName, packet.currentHouse, packet.currentHouseWords);
    }
}
