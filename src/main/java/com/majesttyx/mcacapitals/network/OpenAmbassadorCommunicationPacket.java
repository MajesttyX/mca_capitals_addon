...
public final class OpenAmbassadorCommunicationPacket {
    ...
    public static OpenAmbassadorCommunicationPacket decode(FriendlyByteBuf buffer) {
        ...
        for (int index = 0; index < actionCount; index++) {
            actions.add(new Action(buffer.readUtf(), buffer.readUtf(), buffer.readUtf(), buffer.readBoolean()));
        }
    ...
    public record Entry(String heading, String lineOne, String lineTwo, String lineThree,
                        String buttonLabel, String command, boolean enabled, String disabledReason) {
        public Entry {
            heading = safe(heading); lineOne = safe(lineOne); lineTwo = safe(lineTwo); lineThree = safe(lineThree);
            buttonLabel = safe(buttonLabel); command = safe(command); disabledReason = safe(disabledReason);
        }
    }

    public record Action(String label, String description, String command, boolean enabled) {
        public Action {
            label = safe(label); description = safe(description); command = safe(command);
        }
    }
}
