...
public final class OpenSealedPurseCaseSelectionPacket {
    ...
    public List<CaseEntry> cases() { return List.copyOf(cases); }
    ...
    public static OpenSealedPurseCaseSelectionPacket decode(FriendlyByteBuf buffer) {
        ...
        if (size < 0 || size > MAX_CASES) throw new IllegalArgumentException("Invalid Sealed Purse case count: " + size);
        List<CaseEntry> cases = new ArrayList<>(size);
        for (int i = 0; i < size; i++) cases.add(new CaseEntry(buffer.readUUID(), buffer.readUtf(), buffer.readUtf()));
    ...
    public static void handle(OpenSealedPurseCaseSelectionPacket packet) {
        if (packet != null) SealedPurseCaseSelectionClient.open(packet);
    }
