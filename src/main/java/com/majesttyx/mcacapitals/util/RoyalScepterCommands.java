...
public final class RoyalScepterCommands {
    ...
    private static int appointMasterOfLaws(CommandSourceStack source, String rawVillagerId) {
        ServerPlayer player = getPlayer(source);
        if (player == null) {
            source.sendFailure(Component.literal("Only a player can use this."));
            return 0;
        ...
        if (!CapitalBuildingService.hasPrison(level, capital)) {
            source.sendFailure(Component.literal("A recognized Prison is required before appointing a Master of Laws."));
            return 0;
        }

        if (villagerId.equals(capital.getMasterOfLaws())) {
            source.sendFailure(Component.literal(resolveName(level, villagerId) + " already holds the office of Master of Laws."));
            return 0;
        }

        if (!CapitalMasterOfLawsService.isEligibleCandidate(level, capital, villagerId, residents)) {
            source.sendFailure(Component.literal("That villager is not eligible to serve as Master of Laws."));
            return 0;
        }

        if (!CapitalMasterOfLawsService.appointMasterOfLaws(level, capital, villagerId, residents)) {
            source.sendFailure(Component.literal("The Master of Laws appointment could not be completed."));
            return 0;
        }

        String name = resolveName(level, villagerId);
        source.sendSuccess(() -> Component.literal(name + " was appointed Master of Laws."), false);
        return 1;
    }

    private static int appointAmbassador(CommandSourceStack source, String rawVillagerId) {
        ServerPlayer player = getPlayer(source);
        if (player == null) {
            source.sendFailure(Component.literal("Only a player can use this."));
            return 0;
        ...
        if (!CapitalBuildingService.hasAmbassadorBuildings(level, capital)) {
            source.sendFailure(Component.literal("A recognized Inn and Storage are required before appointing an Ambassador."));
            return 0;
        }

        if (CapitalAmbassadorService.isAmbassador(level, capital, villagerId)) {
            source.sendFailure(Component.literal(resolveName(level, villagerId) + " already holds the office of Ambassador."));
            return 0;
        }

        if (!CapitalAmbassadorService.isEligibleCandidate(level, capital, villagerId, residents)) {
            source.sendFailure(Component.literal("That villager is not eligible to serve as Ambassador."));
            return 0;
        }

        if (!CapitalAmbassadorService.appointAmbassador(level, capital, villagerId, residents)) {
            source.sendFailure(Component.literal("The Ambassador appointment could not be completed."));
            return 0;
        }

        String name = resolveName(level, villagerId);
        source.sendSuccess(() -> Component.literal(name + " was appointed Ambassador."), false);
        return 1;
    }
