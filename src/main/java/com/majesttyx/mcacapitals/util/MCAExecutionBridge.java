...
public final class MCAExecutionBridge {
    ...
    private static final String[] PROFESSION_CLASS_NAMES = {
            "forge.net.conczin.mca.ProfessionsMCA"
    };
    private MCAExecutionBridge() {
    }

    public static boolean markForExecution(
            ServerLevel level,
            UUID villagerId
    ) {
        VillagerProfession outlaw =
                resolveOutlawProfession();
    ...
    public static boolean clearExecutionMark(
            ServerLevel level,
            UUID villagerId
    ) {
        if (!isMarkedForExecution(
                level,
                villagerId
        )) {
            return false;
    ...
    public static boolean isMarkedForExecution(
            ServerLevel level,
            UUID villagerId
    ) {
        ...
        VillagerProfession current =
                getProfession(entity);
        ...
        return current != null
                && outlaw != null
                && current.equals(outlaw);
    }

    private static boolean setProfession(
            ServerLevel level,
            UUID villagerId,
    ...
    private static VillagerProfession getProfession(
            Entity entity
    ) {
        if (entity == null) {
            return null;
    ...
    private static VillagerProfession resolveOutlawProfession(
        ...
        try {
            Class<?> professionsClass =
                    Class.forName(className);

            Field outlawField =
                    professionsClass.getField(
                            "OUTLAW"
                    );

            Object raw =
                    outlawField.get(null);
