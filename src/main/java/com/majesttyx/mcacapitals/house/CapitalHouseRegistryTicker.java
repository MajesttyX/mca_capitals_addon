...
public final class CapitalHouseRegistryTicker {
    private static final int SYNC_INTERVAL_TICKS=20; private CapitalHouseRegistryTicker(){}
    public static void onLevelTick(ServerLevel level){
        if(level==null||level.getGameTime()%SYNC_INTERVAL_TICKS!=0L) return;
        for(CapitalRecord capital:CapitalManager.getAllCapitalsSnapshot().values()){
            if(capital==null||capital.getCapitalId()==null||capital.getState()!=CapitalState.ACTIVE||!CapitalManager.isCapitalInLevel(capital,level)) continue;
