...
public final class CapitalNaturalDukedomService {
    ...
    public static boolean tick(ServerLevel level, CapitalRecord capital, Set<UUID> residents) {
        if (level == null || capital == null || residents == null || residents.isEmpty() || capital.getState() != CapitalState.ACTIVE) {
            return false;
        }
        ...
        CapitalChronicleService.addEntry(level, capital, name + " was raised to " + (female ? "Duchess" : "Duke") + " by the growth of the great houses.");
        CapitalDataAccess.markDirty(level);
        return true;
    }

    private static int countNaturalDukes(CapitalRecord capital, Set<UUID> residents) {
        int count = 0;
    ...
    private static UUID selectCandidate(ServerLevel level, CapitalRecord capital, Set<UUID> residents) {
        ...
        for (UUID resident : residents) {
            if (resident == null || blocked.contains(resident)) {
                continue;
            }
