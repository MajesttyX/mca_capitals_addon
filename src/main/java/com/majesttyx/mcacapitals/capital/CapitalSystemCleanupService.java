...
public final class CapitalSystemCleanupService {
    ...
    public static boolean removeCapital(
        ...
        changed |= CapitalDiplomacyDataAccess.removeCapital(level, capitalId);
        changed |= CapitalAgreementDataAccess.removeCapital(level, capitalId);
        changed |= CapitalCampaignDataAccess.removeCapital(level, capitalId);
        changed |= CapitalInterregnumDataAccess.remove(level, capitalId);
        changed |= CapitalJusticeDataAccess.removeCapital(level, capitalId);
        changed |= CapitalWarDataAccess.removeCapital(level, capitalId);
        changed |= CapitalRefugeeDataAccess.removeCapital(level, capitalId);
        changed |= PendingVillagerBetrothalAccess.removeCapital(level, capitalId);

        CapitalAmbassadorService.clearCapital(level, capitalId);
    ...
    private static void clearPlayerTitles(
        ...
        if (playerSovereignId != null) {
            PlayerCapitalTitleService.clear(level, playerSovereignId, capital.getCapitalId());
        }

        UUID playerConsortId = capital.getPlayerConsortId();
        if (playerConsortId != null) {
            PlayerCapitalTitleService.clear(level, playerConsortId, capital.getCapitalId());
        }
    }
}
