...
public final class CapitalCrownStandingService {
    ...
    private static CrownStanding resolveStanding(
            ServerLevel level,
            CapitalRecord capital,
            UUID villagerId,
            boolean sovereignChanged
    ) {
        if (capital == null || villagerId == null) {
            return CrownStanding.FRIEND_OF_CROWN;
        }
        ...
        int roll = Math.floorMod(
                (capital.getCapitalId().toString()
                        ...
                        + villagerId
                        + ":"
                        + capital.getSovereign()).hashCode(),
                100
        );

        return roll < enemyChance
    ...
    private static int enemyChance(ServerLevel level, CapitalRecord capital, UUID villagerId) {
        ...
        if (villagerId.equals(capital.getHand())
                ...
                || villagerId.equals(capital.getHerald())
