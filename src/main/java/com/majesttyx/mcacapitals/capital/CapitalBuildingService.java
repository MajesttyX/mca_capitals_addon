...
public final class CapitalBuildingService {
    ...
    public static List<AABB> getPrisonBounds(ServerLevel level, CapitalRecord capital) {
        ...
        return MCAIntegrationBridge.getBuildingBoundsOfType(level, capital.getVillageId(), PRISON);
    }
    ...
    public static List<BlockPos> getPrisonCenters(ServerLevel level, CapitalRecord capital) {
        ...
        return MCAIntegrationBridge.getBuildingCentersOfType(level, capital.getVillageId(), PRISON);
    }

    private static int countBuildings(ServerLevel level, CapitalRecord capital, String buildingType) {
        if (level == null || capital == null || capital.getVillageId() == null || buildingType == null || buildingType.isBlank()) {
            return 0;
        }
        return MCAIntegrationBridge.countBuildingsOfType(level, capital.getVillageId(), buildingType);
    }
}