...
public final class CapitalAgreementDataAccess {
    ...
    public static boolean renewTradeAgreement(
            ServerLevel level,
            UUID firstCapitalId,
            UUID secondCapitalId
    ) {
        ...
        CapitalTradeAgreement agreement = getTradeAgreement(
                level,
                firstCapitalId,
                secondCapitalId
        );
        ...
        agreement.renewTerm(level.getGameTime());
        get(level).setDirty();
        return true;
    }

    public static void markTradeAgreementChanged(
            ServerLevel level
    ) {
        if (level != null) {
            get(level).setDirty();
