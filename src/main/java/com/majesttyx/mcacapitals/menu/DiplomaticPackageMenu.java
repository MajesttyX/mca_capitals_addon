...
public final class DiplomaticPackageMenu extends AbstractContainerMenu {
    ...
    private static final int PACKAGE_SLOT_COUNT =
            DiplomaticPackageItem.SLOT_COUNT;
    ...
    public DiplomaticPackageMenu(
        ...
        this.packageInventory =
                new SimpleContainer(PACKAGE_SLOT_COUNT);

        loadContents();

        packageInventory.addListener(
                ignored -> saveContents()
        );
    ...
    private void loadContents() {
        ...
        List<ItemStack> stored =
                DiplomaticPackageItem.readContents(packageStack);
        ...
        for (int slot = 0;
             slot < PACKAGE_SLOT_COUNT;
             slot++) {
            ItemStack stack = slot < stored.size()
                    ? stored.get(slot).copy()
                    : ItemStack.EMPTY;

            packageInventory.setItem(slot, stack);
        }
    ...
    private void saveContents() {
        ...
        for (int slot = 0;
            ...
            saved.set(
                    slot,
                    packageInventory.getItem(slot).copy()
            );
        }

        DiplomaticPackageItem.writeContents(
                packageStack,
                saved
        );
    }
    ...
    private ItemStack findPackageStack() {
        ...
        if (DiplomaticPackageItem.hasPackageId(
                held,
                packageId
        )) {
            return held;
    ...
    private final class PackageAwarePlayerSlot extends Slot {
        ...
        public boolean mayPickup(Player player) {
            return !DiplomaticPackageItem.hasPackageId(
                    getItem(),
                    packageId
            );
        }
